...
struct BenchmarkScale {
	...
	int runs() const { return smoke ? 5 : 7; }

	int n_graph() const { return smoke ? 50'000 : 1'000'000; }
	int n_tree() const { return smoke ? 50'000 : 1'000'000; }
	int n_list() const { return smoke ? 500'000 : 5'000'000; }
	int n_perm() const { return smoke ? 500'000 : 5'000'000; }
	int n_geom() const { return smoke ? 10'000 : 1'000'000; }
	int n_ortho_geom() const { return smoke ? 200'000 : 1'000'000; }
	long long geom_max() const { return smoke ? 1'000'000 : 3'000'000; }
	int n_bipartite_side() const { return smoke ? 300 : 1'000; }
	int m_bipartite() const { return smoke ? 25'000 : 500'000; }

	int partition_n() const { return smoke ? 100'000 : 5'050'000; }
	int partition_fixed_n() const { return smoke ? 1'000'000 : 58'000'000; }
	int partition_bounded_n() const { return smoke ? 800 : 4'200; }
	int partition_fast_k() const { return smoke ? 500'000 : 3'000'000; }
};
...
std::vector<benchmark::CaseResult> run_all(const BenchmarkScale &scale) {
	...
	run("tgen::graph::gen", "", scale.smoke ? "n=5e4, m=5e4" : "n=1e6, m=1e6",
		"tgen::wgraph::gen", [=] { consume_graph(tgen::graph(n, n).gen()); });

	run("tgen::graph::gen", " (directed)",
		scale.smoke ? "n=5e4, m=5e4, directed" : "n=1e6, m=1e6, directed",
		"tgen::wgraph::gen",
	...
	run("tgen::graph::gen_skewed", " (m=2n)",
		scale.smoke ? "n=5e4, m=1e5, elongation=1e2, spread=4"
	...
	int regex_reps = scale.smoke ? 50'000 : 2'500'000;
	...
	run("tgen::geometry::random_simple_polygon", "",
		scale.smoke ? "n=1e4, min=0, max=1e6, strict=false"
	...
	run("tgen::geometry::random_points_general_position", "",
		scale.smoke ? "n=1e4, min=0, max=1e6" : "n=1e6, min=0, max=3e6",
	...
	run("tgen::geometry::random_convex_polygon", " (strict)",
		scale.smoke ? "n=1e4, min=0, max=1e12, strict=true"
	...
	run("tgen::geometry::random_orthogonal_polygon", "",
		scale.smoke ? "n=2e5, min=0, max=1e6, strict=false"
					: "n=1e6, min=0, max=3e6, strict=false",
	...
	run("tgen::geometry::random_orthogonal_polygon", " (strict)",
		scale.smoke ? "n=2e5, min=0, max=1e6, strict=true"
	...
	run("tgen::math::gen_partition_fixed_size", " (bounded)",
		scale.smoke ? "n=800, k=800, part_left=0, part_right=20"
	...
	run("tgen::math::gen_partition_fixed_size_fast", "",
		scale.smoke ? "n=1e18, k=5e5, part_left=0"
